...
function enterKey(e) {
  ...
  if (e.keyCode === 13) {
    ...
    addLine("[jalim@archrx5500m]~$" + command.innerHTML, "no-animation", 0);
...
function commander(cmd) {
  switch (cmd.toLowerCase()) {
    ...
    case "email":
      addLine(
        'Opening mailto:<a href="mailto:mohamadjalim05@gmail.com"> mohamadjalim05@gmail.com</a>...',
        "color2",
        80,
      );
      newTab(email);
...
function addLine(text, style, time,scroll=true) {
  let t = "";
...
function loopLines(name, style, time,scroll=true) {
  name.forEach(function (item, index) {
    addLine(item, style, index * time, scroll);
  });
  if (scroll) {
    setTimeout(function () {
      scrollToBottom();
